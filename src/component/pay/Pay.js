  ...
  const load = () => {
    ...
    if (id !== undefined) {
      ...
      getRequest(url)
        ...
        .catch(() => {
          toast.error("پیش فاکتوری جهت پرداخت یافت نشد. لطفا با واحد پشتیبانی تماس بگیرید");
        });
    }
  };
    ...
    patchRequest("/customers/save?id=" + state.id, tmp)
      .then((data) => {
        ...
        if (result.error) {
          ...
          toast.success("اطلاعات با موفقیت ثبت شد. لطفا کمی صبر کنید تا به درگاه بانک متصل شوید", { autoClose: false, position: "top-center" });
    ...
    <>
      ...
      {state.id != null &&
        <Form onSubmit={saveData}>
          <Row>
            <Col md={12}>
              <label className={"mt-3"}>پیش فاکتور شماره: <b>{inv.id}</b></label>
            </Col>
          </Row>
          <hr />
          <Row>
            <Col md={12}>
              <label className={"mt-1"}>خدمت ارائه شده: <b>{inv.service_name}</b></label>
            </Col>
          </Row>
          <Row>
            <Col>
              <label className={"mt-2"}>نام شخص حقیقی/حقوقی: <b>{state.name}</b></label>
            </Col>
          </Row>
          <hr />
          ...
          <Row>
            ...
            <Col>
              <Form.Group className="mb-3" controlId="owner">
                <Form.Label>{"نام خدمت گیرنده/نام نمایندگینام خدمت گیرنده"}</Form.Label>
                <Form.Control
                  type="text"
                  required={true}
                  onChange={(e) => {
          ...
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3" controlId="economicalـnumber">
                ...
                <Form.Control
                  ...
                  onChange={(e) => {
                    let tmp = { ...state };
                    tmp.economicalـnumber = setPersianNumberToEnglish(e.target.value);
                    setState(tmp);
                  }}
            ...
            <Col md={6}>
              <Form.Group className="mb-3" controlId="nuique_id">
                <Form.Label>{state.is_personal == 1 ? "کد ملی" : "شناسه ملی"}</Form.Label>
                <Form.Control
                  style={{ direction: "ltr" }}
                  type="text"
                  required={false}
          ...
          <Row>
            <Col>
              <ul>
                ...
                <li><label className="mt-2">
                  مبلغ ارزش افزوده: <b>{numberWithCommas(inv.pay_arzesh_afzodeh)}</b> ریال
                  {inv.with_tax == 0 && <>&nbsp;<lable style={{ "color": "red" }}>(پیش فاکتور بدون ارزش افزوده صادر شده است)</lable></>}
                </label>
                </li>
                <li>
                  <label className="mt-2">
                    مبلغ قابل پرداخت: <b>{numberWithCommas(inv.price + inv.pay_arzesh_afzodeh)}</b> ریال
                  </label>
                </li>
              </ul>
            ...
            <Row>
              <Col className="mb-3" md={12}>
                <Button variant="primary" type="submit" disabled={isload}>
                  تایید اطلاعات و پرداخت <Spinner
                    as="span"
                    animation="border"
                    size="sm"
                    role="status"
